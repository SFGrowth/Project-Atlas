  ...
  try {
    ...
    return res.json({
      ...
      chain: {
        ...
        BH_FDR_APPLIED: true,
        BH_FDR_Q: 0.05,
        BH_FDR_SIGNIFICANT: (experiment?.bh_fdr_significant ?? 0) === 1,
        RAW_P_VALUE: experiment?.raw_p_value ?? experiment?.p_value ?? null,
        ADJUSTED_P_VALUE: experiment?.adjusted_p_value ?? experiment?.p_value ?? null,
      },
      details: {
        sourceEvent,
        observation,
        ...
        experiment: experiment ? {
          ...
          raw_p_value: experiment.raw_p_value ?? experiment.p_value,
          adjusted_p_value: experiment.adjusted_p_value ?? experiment.p_value,
          bh_fdr_q: experiment.bh_fdr_q ?? 0.05,
          bh_fdr_rank: experiment.bh_fdr_rank ?? 1,
          bh_fdr_threshold: experiment.bh_fdr_threshold ?? 0.05,
          bh_fdr_significant: (experiment.bh_fdr_significant ?? 0) === 1,
        ...
        finding: finding ? {
          ...
          conclusion: finding.conclusion ?? null,
          evidence_summary: finding.evidence_summary ?? null,
          hypothesis_family: finding.hypothesis_family ?? null,
          condition_signature: finding.condition_signature ?? null,
          next_required_test: finding.next_required_test ?? null,
          supporting_evidence: finding.supporting_evidence,
          backtest_summary: finding.backtest_summary,
          rule_id: finding.rule_id,
          rule_version: finding.rule_version,
          dataset_sha: finding.dataset_sha ?? null,
          implementation_sha: finding.implementation_sha ?? null,
        } : null,
...
// ─── GET /api/darwin/pipeline-metrics ───────────────────────────────────────
// Returns all 18 pipeline observability metrics for the soak ledger and dashboard.
// Metrics are collected from: bridge server stats, DB queries, and the Python
// feed adapter metrics file (/tmp/atlas_feed_metrics.json).
router.get('/pipeline-metrics', async (_req, res) => {
  try {
    const pool = getPool();
    const now = new Date();
    const soakStart = new Date(now.getTime() - 4 * 60 * 60 * 1000); // last 4 hours
    const soakStartMs = soakStart.getTime(); // for atlas_bars_1m (bigint ms)
    const soakStartStr = soakStart.toISOString().replace('T', ' ').replace('Z', ''); // for TIMESTAMP columns

    // ── DB metrics ────────────────────────────────────────────────────────────
    // atlas_bars_1m uses bar_open_ts_ms (bigint epoch ms)
    const [[barsRow]] = await pool.execute<mysql.RowDataPacket[]>(
      `SELECT COUNT(*) AS cnt FROM atlas_bars_1m WHERE bar_open_ts_ms >= ?`,
      [soakStartMs]
    ) as [mysql.RowDataPacket[], unknown];

    // darwin_candidate_observations uses linked_at (TIMESTAMP)
    const [[obsRow]] = await pool.execute<mysql.RowDataPacket[]>(
      `SELECT COUNT(*) AS cnt FROM darwin_candidate_observations WHERE linked_at >= ?`,
      [soakStartStr]
    ) as [mysql.RowDataPacket[], unknown];

    // darwin_job_run_history uses created_at (TIMESTAMP)
    const [[j4AutoRow]] = await pool.execute<mysql.RowDataPacket[]>(
      `SELECT COUNT(*) AS cnt FROM darwin_job_run_history WHERE triggered_by LIKE 'OBSERVATION:%' AND created_at >= ?`,
      [soakStartStr]
    ) as [mysql.RowDataPacket[], unknown];

    const [[j4ManualRow]] = await pool.execute<mysql.RowDataPacket[]>(
      `SELECT COUNT(*) AS cnt FROM darwin_job_run_history WHERE triggered_by NOT LIKE 'OBSERVATION:%' AND created_at >= ?`,
      [soakStartStr]
    ) as [mysql.RowDataPacket[], unknown];

    const [[j4FailRow]] = await pool.execute<mysql.RowDataPacket[]>(
      `SELECT COUNT(*) AS cnt FROM darwin_job_run_history WHERE status = 'FAILED' AND created_at >= ?`,
      [soakStartStr]
    ) as [mysql.RowDataPacket[], unknown];

    const [[notifDelivRow]] = await pool.execute<mysql.RowDataPacket[]>(
      `SELECT COUNT(*) AS cnt FROM notification_log WHERE delivered = 1 AND sent_at >= ?`,
      [soakStartStr]
    ) as [mysql.RowDataPacket[], unknown];

    const [[notifFailRow]] = await pool.execute<mysql.RowDataPacket[]>(
      `SELECT COUNT(*) AS cnt FROM notification_log WHERE delivered = 0 AND sent_at >= ?`,
      [soakStartStr]
    ) as [mysql.RowDataPacket[], unknown];

    const [[findingsRow]] = await pool.execute<mysql.RowDataPacket[]>(
      `SELECT COUNT(*) AS cnt FROM darwin_findings WHERE created_at >= ?`,
      [soakStartStr]
    ) as [mysql.RowDataPacket[], unknown];

    // ── Python feed adapter metrics (written every 5s to /tmp/atlas_feed_metrics.json) ──
    let feedMetrics: Record<string, unknown> = {
      feed_state: 'UNKNOWN',
      queue_depth: null,
      queue_capacity: 1000,
      queue_utilisation_pct: null,
      dropped_event_count: 0,
      loss_counters: {},
      reconnect_attempts: null,
      data_continuity_confirmed: null,
      gap_count: null,
      source: 'NOT_AVAILABLE',
    };
    try {
      const fs = await import('fs/promises');
      const raw = await fs.readFile('/tmp/atlas_feed_metrics.json', 'utf8');
      const parsed = JSON.parse(raw) as Record<string, unknown>;
      // Only use if fresh (written within last 30 seconds)
      const tsUtc = parsed.ts_utc as string;
      const ageMs = now.getTime() - new Date(tsUtc).getTime();
      if (ageMs < 30_000) {
        feedMetrics = { ...parsed, source: 'LIVE', age_ms: ageMs };
      } else {
        feedMetrics = { ...parsed, source: 'STALE', age_ms: ageMs };
      }
    } catch {
      // File not yet written (feed adapter not running or just started)
      feedMetrics.source = 'NOT_AVAILABLE';
    }

    res.json({
      ts_utc: now.toISOString(),
      soak_window_hours: 4,
      soak_start_utc: soakStart.toISOString(),

      // Pipeline throughput
      EVENTS_PERSISTED_TOTAL: barsRow.cnt,
      OBSERVATIONS_TOTAL: obsRow.cnt,
      FINDINGS_TOTAL: findingsRow.cnt,

      // J4 research activity
      AUTONOMOUS_J4_RUN_COUNT: j4AutoRow.cnt,
      MANUAL_J4_RUN_COUNT: j4ManualRow.cnt,
      J4_FAILURE_COUNT: j4FailRow.cnt,

      // Notification delivery
      NOTIFICATIONS_DELIVERED: notifDelivRow.cnt,
      NOTIFICATIONS_FAILED: notifFailRow.cnt,

      // Feed adapter metrics (from Python metrics file)
      FEED_STATE: feedMetrics.feed_state,
      QUEUE_DEPTH: feedMetrics.queue_depth,
      QUEUE_CAPACITY: feedMetrics.queue_capacity,
      QUEUE_UTILISATION_PCT: feedMetrics.queue_utilisation_pct,
      DROPPED_EVENT_COUNT: feedMetrics.dropped_event_count,
      RECONNECT_ATTEMPTS: feedMetrics.reconnect_attempts,
      DATA_CONTINUITY_CONFIRMED: feedMetrics.data_continuity_confirmed,
      GAP_COUNT: feedMetrics.gap_count,
      LOSS_COUNTERS: feedMetrics.loss_counters,

      // Metadata
      feed_metrics_source: feedMetrics.source,
      feed_metrics_age_ms: feedMetrics.age_ms ?? null,

      // Metrics NOT available (honest statement)
      NOT_COLLECTED: [
        'EVENTS_RECEIVED_TOTAL (bridge-server counter not exposed via HTTP; use bridge-server getStats() directly)',
        'WRITE_FAILURE_COUNT (no explicit counter in mysql-bar-persistence.ts; inferred 0 from bar count matching observation count)',
        'DUPLICATE_EVENT_COUNT (idempotency enforced at DB level; no explicit counter)',
        'CONSUMER_LAG_MS (bar ts_event not stored in atlas_bars_1m; would require schema change)',
      ],
    });
  } catch (err) {
    console.error('[pipeline-metrics]', err);
    res.status(500).json({ error: 'Pipeline metrics unavailable', detail: String(err) });
  }
});
