...
export async function findExistingCandidate(signature: string): Promise<{
  ...
  const pool = getPool();
  const [rows] = await pool.execute<mysql.RowDataPacket[]>(`
    SELECT candidate_id, candidate_version, governance_stage, last_observed, experiment_id
...
export async function sendFindingNotification(params: {
  ...
  if (botToken && chatId) {
    try {
      ...
      const { markNotificationDelivery } = await import('../_core/notificationRetryService');
      ...
      // Correctly mark delivery: only delivered=1 when Telegram confirms; otherwise schedule retry
      await markNotificationDelivery(notificationId, tgResult, 1 /* CRITICAL */);
    } catch (err) {
