...
import { InsertUser, pipelineReports, users } from "../drizzle/schema";
import { InsertPipelineReport } from "../drizzle/schema";
...
export async function getPipelineReportCount(): Promise<number> {
  const db = await getDb();
  if (!db) return 0;
  const result = await db.select().from(pipelineReports);
