...
class DatabentoFeedAdapter:
    ...
    async def run(self) -> None:
        ...
        # Start the bridge sender task and pipeline metrics writer task
        sender_task = asyncio.create_task(self._bridge_sender_loop())
        metrics_task = asyncio.create_task(self._metrics_writer_loop())
    ...
    async def _metrics_writer_loop(self) -> None:
        """Write pipeline metrics to /tmp/atlas_feed_metrics.json every 5 seconds.
        Read-only monitoring — does not affect pipeline behaviour or data flow.
        Atomic write via rename to prevent partial reads.
        """
        METRICS_PATH = "/tmp/atlas_feed_metrics.json"
        while not self._stopped:
            try:
                metrics = {
                    "ts_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                    "feed_state": self._feed_state.name,
                    "queue_depth": self._queue.qsize(),
                    "queue_capacity": BRIDGE_QUEUE_MAX,
                    "queue_utilisation_pct": round(
                        self._queue.qsize() / BRIDGE_QUEUE_MAX * 100, 2
                    ),
                    "loss_counters": dict(self._loss_counters),
                    "dropped_event_count": sum(self._loss_counters.values()),
                    "reconnect_attempts": self._reconnect_attempts,
                    "data_continuity_confirmed": self._data_continuity_confirmed,
                    "gap_count": len(self._gaps),
                }
                tmp = METRICS_PATH + ".tmp"
                with open(tmp, "w") as f:
                    json.dump(metrics, f)
                os.replace(tmp, METRICS_PATH)  # atomic write
            except Exception as exc:
                logger.debug("[FeedAdapter] metrics writer error: %s", type(exc).__name__)
