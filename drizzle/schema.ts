...
export const strategyRegistry = mysqlTable("strategy_registry", {
  ...
  notes: text("notes"),
  rejectionReason: text("rejection_reason"),
  // Pine Script metadata (Sprint 117)
  pineEnabled: boolean("pine_enabled").default(false),
  pineStrategyKey: varchar("pine_strategy_key", { length: 32 }),
  pineVersion: varchar("pine_version", { length: 16 }),
  pineRuleHash: varchar("pine_rule_hash", { length: 128 }),
  pineParityStatus: varchar("pine_parity_status", { length: 32 }).default("NOT_CONFIGURED"),
  pineLastVerifiedAt: bigint("pine_last_verified_at", { mode: "number" }),
  pineChartColour: varchar("pine_chart_colour", { length: 16 }),
  pineWebhookEnabled: boolean("pine_webhook_enabled").default(false),
  pineLastWebhookAt: bigint("pine_last_webhook_at", { mode: "number" }),
  pineLastSignalAt: bigint("pine_last_signal_at", { mode: "number" }),
  pineLastSignalDirection: varchar("pine_last_signal_direction", { length: 8 }),
  pineLastSignalScore: decimal("pine_last_signal_score", { precision: 8, scale: 2 }),
  pineKnownGaps: text("pine_known_gaps"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
/**
 * gap_candidates — individual portfolio/research/execution gaps identified
 * by the autonomous Gap Discovery Engine.
 *
 * dimension:
 *   MARKET_BEHAVIOUR | REGIME_COVERAGE | LOW_CONFIDENCE_LAW | BEHAVIOUR_LIBRARY |
 *   SEQUENCE_LIBRARY | UNDERPERFORMING_MODEL | RESEARCH_BOTTLENECK |
 *   EXECUTION_BOTTLENECK | DASHBOARD_BLIND_SPOT | DATA_QUALITY |
 *   CORRELATION_WEAKNESS | RISK_ALLOCATION
 *
 * status:
 *   OPEN | INVESTIGATING | RESOLVED | DEFERRED | REJECTED
 */
export const gapCandidates = mysqlTable("gap_candidates", {
  id:                   int("id").autoincrement().primaryKey(),
  dimension:            varchar("dimension", { length: 64 }).notNull(),
  title:                varchar("title", { length: 256 }).notNull(),
  description:          text("description").notNull(),
  evidence:             text("evidence"),                              // supporting data/observations
  impactScore:          decimal("impact_score", { precision: 5, scale: 1 }).notNull().default("0.0"),   // 0–10
  confidenceScore:      decimal("confidence_score", { precision: 5, scale: 1 }).notNull().default("0.0"), // 0–10
  effortEstimate:       varchar("effort_estimate", { length: 32 }),   // LOW | MEDIUM | HIGH | SPRINT
  expectedBenefit:      text("expected_benefit"),
  expectedRiskReduction: text("expected_risk_reduction"),
  priorityRank:         int("priority_rank"),                         // 1 = highest priority
  status:               mysqlEnum("status", ["OPEN", "INVESTIGATING", "RESOLVED", "DEFERRED", "REJECTED"]).notNull().default("OPEN"),
  relatedStrategyId:    varchar("related_strategy_id", { length: 32 }), // if gap is strategy-specific
  relatedSprintId:      varchar("related_sprint_id", { length: 32 }), // if gap is linked to a sprint
  resolvedAt:           bigint("resolved_at", { mode: "number" }),
  resolvedNotes:        text("resolved_notes"),
  sourceRunId:          int("source_run_id"),                         // gap_discovery_reports.id that created this
  autoGenerated:        boolean("auto_generated").notNull().default(true),
  createdAt:            timestamp("created_at").defaultNow().notNull(),
  updatedAt:            timestamp("updated_at").defaultNow().notNull(),
});
export type GapCandidate = typeof gapCandidates.$inferSelect;
export type InsertGapCandidate = typeof gapCandidates.$inferInsert;

/**
 * gap_discovery_reports — weekly automated gap analysis reports.
 * One row per run (typically weekly, Sunday 18:00 ET).
 */
export const gapDiscoveryReports = mysqlTable("gap_discovery_reports", {
  id:                   int("id").autoincrement().primaryKey(),
  reportDate:           varchar("report_date", { length: 16 }).notNull(), // YYYY-MM-DD
  weekStartDate:        varchar("week_start_date", { length: 16 }),
  weekEndDate:          varchar("week_end_date", { length: 16 }),
  // Summary counts
  totalGapsIdentified:  int("total_gaps_identified").notNull().default(0),
  newGapsThisWeek:      int("new_gaps_this_week").notNull().default(0),
  resolvedThisWeek:     int("resolved_this_week").notNull().default(0),
  openGaps:             int("open_gaps").notNull().default(0),
  // Top findings (JSON arrays)
  top10PortfolioGaps:   text("top_10_portfolio_gaps"),                // JSON: GapCandidate[]
  top10ResearchOpps:    text("top_10_research_opps"),                 // JSON: GapCandidate[]
  topEngineeringImprovements: text("top_engineering_improvements"),   // JSON: GapCandidate[]
  topExecutionImprovements:   text("top_execution_improvements"),     // JSON: GapCandidate[]
  topDashboardImprovements:   text("top_dashboard_improvements"),     // JSON: GapCandidate[]
  // Autonomous question answers (JSON object)
  autonomousQuestionAnswers: text("autonomous_question_answers"),     // JSON: {question: answer}[]
  // Estimated portfolio improvement
  estimatedPortfolioImprovementPct: decimal("estimated_portfolio_improvement_pct", { precision: 5, scale: 2 }),
  recommendedNextPriority: text("recommended_next_priority"),
  // Raw analysis data
  analysisDataJson:     text("analysis_data_json"),                   // full analysis payload
